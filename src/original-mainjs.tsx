...
import layouts from './util/layouts-original.json';
...
import htkai from './fonts/fz-ht-kai.json';
import heiti from './fonts/heiti.json';
...
const scene: THREE.Scene = new THREE.Scene(); // the scene that holds all the objects
const camera: THREE.Camera = new THREE.OrthographicCamera(-7,7,7,-7,0.1,100); // responsible for looking at the objects
const renderer: THREE.WebGLRenderer = new THREE.WebGLRenderer(); // renders the result on the page based on scene and camera
const raycaster: THREE.Raycaster = new THREE.Raycaster(); // object for raycasting (picking the contact point of cursor & object)
const pointer: THREE.Vector2 = new THREE.Vector2(); // the position of the pointer on the screen
...
let selected: null | Piece = null;
let highlighted: null | Piece = null;
let myTeam = 1;
let start = Date.now();
